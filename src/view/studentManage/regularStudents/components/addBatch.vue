<template>
  <el-dialog v-model="dialogVisible" title="批量新增" width="450px" class="dialogContainer">
      ...
      <template v-if="activeStep === '1'">
        <div class="templateContainer">
          ...
          <div class="download" @click="downloadTemplate">
            ...
            <span class="downloadTxt">批量新增学生模板.xlsx</span>
          </div>
        </div>
...
const downloadTemplate = () =>{
  ...
  downloadFileWithLink(`${path}/download/批量新增学生模板.xlsx`, "批量新增学生模板");
  // API.student.downloadTemplate({type: 1}).then(res => {
  //   if(res) {
  //     downloadFileWithUrl(res, '批量新增学生模板下载')
  //   }
  // });
}
...
const uploadFile = (param) => {
  return API.student.importBatchStudent(param).then(res => res);
}
